...
if (argv.devtools) {
    ...
    const exec: typeof tempExec = utils.promisify(require('child_process').exec);
...
contentScriptNames.forEach(name => {
    const existsTsxIndex = fs.existsSync(resolve(sourcePath, `contents/${name}/index.tsx`));

    entry[name] = [resolve(sourcePath, `contents/${name}/index.${existsTsxIndex ? 'tsx' : 'ts'}`)];
});
